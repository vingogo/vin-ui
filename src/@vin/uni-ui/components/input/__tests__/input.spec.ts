...
test('should render clear icon when using clearable prop', async () => {
  ...
  // expect(wrapper.find('.vin-input-clear').exists()).toBeTruthy();
  wrapper.find('.vin-input-clear').trigger('click');
...
test('should clear when event clear', () => {
  ...
  const input = wrapper.find('input');
  const clear = wrapper.find('.vin-input-clear');
...
test('should require', () => {
  ...
  const input = wrapper.find('.vin-input');
  expect(input.classes()).toContain('vin-input-required');
});
